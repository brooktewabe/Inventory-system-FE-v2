...
const SalesHistory = () => {
  ...
  const fetchSalesByPage = async (page, status = "Pending") => {
    try {
      const response = await axios.get(
        `https://apiv2.cnhtc4.com/sales/all-sales?status=${status}&page=${page}&limit=${itemsPerPage}`
      );
  ...
  const fetchSalesByFilter = async (
    ...
    try {
      const response = await axios.get(
        `https://apiv2.cnhtc4.com/sales/search/find?startDate=${
          startDate || ""
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      await axios.patch(`https://apiv2.cnhtc4.com/sales/change-status/${id}`, { status: newStatus });
      // Refetch the data
      if (searchTerm || filterStartDate || filterEndDate) {
